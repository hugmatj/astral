<template>
  <Switch
    ...
    :class="[
      enabledState ? 'bg-brand-600' : 'bg-gray-200',
  ...
  >
    ...
    >
      <span
        :class="[
          enabledState ? 'opacity-0 duration-100 ease-out' : 'opacity-100 duration-200 ease-in',
      ...
      <span
        :class="[
          enabledState ? 'opacity-100 duration-200 ease-in' : 'opacity-0 duration-100 ease-out',
          'absolute inset-0 flex h-full w-full items-center justify-center transition-opacity',
        ]"
        aria-hidden="true"
      >
